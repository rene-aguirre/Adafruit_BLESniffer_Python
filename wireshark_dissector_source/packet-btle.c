...
G_MODULE_EXPORT const gchar version[] = "none";
...
{
  {extern void proto_register_btle (void); proto_register_btle ();}
}

G_MODULE_EXPORT void
plugin_reg_handoff(void)
{
  {extern void proto_reg_handoff_btle (void); proto_reg_handoff_btle ();}
}
#endif
...
static expert_field ei_unknown_ll_control_code = EI_INIT;
...
dissect_feature_set(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    ...
    feature_set_item = proto_tree_add_item (tree, hf_btle_feature_set,    tvb, offset, 8, ENC_NA);
...
dissect_channel_map(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    ...
    map_item = proto_tree_add_item(tree, hf_btle_channel_map, tvb, offset, 5, ENC_NA);
...
dissect_adv_data_attr(proto_tree *tree, tvbuff_t *tvb, packet_info *pinfo)
{
    ...
    switch(type){
        ...
        case index_hf_btle_adv_data_128b_service_uuids:
            // attr_item = proto_tree_add_item(tree, hf_btle_adv_data_attrs[type], tvb, 2, length-1, ENC_NA);
            // attr_tree = proto_item_add_subtree(attr_item, ett_uuids);
            // attr_length_item = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_length, tvb, 0, 1, ENC_LITTLE_ENDIAN);
            // attr_type_item      = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_type, tvb, 1, 1, ENC_LITTLE_ENDIAN);
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_attrs[type], length, ENC_NA);
        ...
        case index_hf_btle_adv_data_service_data:
            ...
            proto_tree_add_item(attr_tree, hf_service_data_value, tvb, 4, length-3, ENC_NA);
            break;
        case index_hf_btle_adv_data_service_data_32b:
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_attrs[type], length, ENC_NA);
            proto_tree_add_item(attr_tree, hf_btle_32b_uuid, tvb, 2, 4, ENC_LITTLE_ENDIAN);
            proto_tree_add_item(attr_tree, hf_service_data_value, tvb, 6, length-5, ENC_NA);
            break;
        case index_hf_btle_adv_data_service_data_128b:
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_attrs[type], length, ENC_NA);
            proto_tree_add_item(attr_tree, hf_btle_128b_uuid, tvb, 2, 16, ENC_NA);
            proto_tree_add_item(attr_tree, hf_service_data_value, tvb, 18, length-17, ENC_NA);
            break;
        ...
        case index_hf_btle_adv_data_appearance:
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_attrs[type], length, ENC_NA);
            proto_tree_add_item(attr_tree, hf_btle_adv_data_attrs[type], tvb, 2, length-1, ENC_NA);
            break;
        ...
        case 255:
            ...
            // attr_length_item = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_length, tvb, 0, 1, ENC_LITTLE_ENDIAN);
            // attr_type_item      = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_type, tvb, 1, 1, ENC_LITTLE_ENDIAN);
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_manufacturer, length, ENC_NA);
            proto_tree_add_item(attr_tree, hf_btle_adv_data_manufacturer, tvb, 2, length-1, ENC_NA);
            break;
        default:
            // attr_item = proto_tree_add_item(tree, hf_btle_adv_data_unknown, tvb, 2, length-1, ENC_NA);
            // attr_tree = proto_item_add_subtree(attr_item, ett_uuids);
            // attr_length_item = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_length, tvb, 0, 1, ENC_LITTLE_ENDIAN);
            // attr_type_item      = proto_tree_add_item(attr_tree, hf_btle_adv_data_attr_type, tvb, 1, 1, ENC_LITTLE_ENDIAN);
            attr_tree = add_adv_data_attr(tree, tvb, hf_btle_adv_data_unknown, length, ENC_NA);
            proto_tree_add_item(attr_tree, hf_btle_adv_data_unknown, tvb, 2, length-1, ENC_NA);
            break;
    }
}
...
dissect_scan_rsp(proto_tree *tree, tvbuff_t *tvb, packet_info *pinfo, int offset, int datalen)
{
    ...
    data_item = proto_tree_add_item(tree, hf_btle_scan_rsp_data, tvb, offset + 6, datalen, ENC_NA);
    data_tree = proto_item_add_subtree(data_item, ett_adv_data);
...
dissect_ll_enc_req(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    ...
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_req_rand, tvb, offset + 1,  8, ENC_NA);
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_req_ediv, tvb, offset + 9,  2, ENC_NA);
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_req_skdm, tvb, offset + 11, 8, ENC_NA);
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_req_ivm,  tvb, offset + 19, 4, ENC_NA);
}
...
dissect_ll_enc_rsp(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    ...
    // ll_enc_rsp_item = proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_rsp, tvb, offset + 1, 12, ENC_NA);
    ...
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_rsp_skds, tvb, offset + 1, 8, ENC_NA);
    proto_tree_add_item(tree, hf_btle_ll_control_ll_enc_rsp_ivs,  tvb, offset + 9, 4, ENC_NA);
}
...
void dissect_ll_channel_map_req(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    // proto_tree_add_item (tree, hf_btle_channel_map,    tvb, offset+ 1, 5, ENC_NA);
    dissect_channel_map(tree, tvb, offset+1);
    proto_tree_add_item (tree, hf_btle_instant,        tvb, offset+ 6, 2, ENC_LITTLE_ENDIAN);
}

void dissect_ll_terminate_ind(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    proto_tree_add_item (tree, hf_btle_error_code,    tvb, offset+ 1, 1, ENC_LITTLE_ENDIAN);
}

void dissect_ll_start_enc_req(proto_tree *tree, tvbuff_t *tvb, int offset)
{

}

void dissect_ll_start_enc_rsp(proto_tree *tree, tvbuff_t *tvb, int offset)
{

}

void dissect_ll_unknown_rsp(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    proto_tree_add_item (tree, hf_btle_unknown_type,    tvb, offset+ 1, 1, ENC_LITTLE_ENDIAN);
}


void dissect_ll_feature_req(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    dissect_feature_set(tree, tvb, offset+1);
    // proto_tree_add_item (tree, hf_btle_feature_set,    tvb, offset+ 1, 8, ENC_NA);
}

void dissect_ll_feature_rsp(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    dissect_feature_set(tree, tvb, offset+1);
    // proto_tree_add_item (tree, hf_btle_feature_set,    tvb, offset+ 1, 8, ENC_NA);
}

void dissect_ll_pause_enc_req(proto_tree *tree, tvbuff_t *tvb, int offset)
{

}

void dissect_ll_pause_enc_rsp(proto_tree *tree, tvbuff_t *tvb, int offset)
{

}

void dissect_ll_version_ind(proto_tree *tree, tvbuff_t *tvb, int offset)
{
...
void dissect_ll_reject_ind(proto_tree *tree, tvbuff_t *tvb, int offset)
{
    proto_tree_add_item (tree, hf_btle_error_code,    tvb, offset+ 1, 1, ENC_LITTLE_ENDIAN);
}

void dissect_ll_control(proto_tree *tree, tvbuff_t *tvb, packet_info *pinfo, int offset, guint8 length)
{
    ...
    if (ll_control_opcode <= 0x0d) {
        ...
        if (length > 1)
            proto_tree_add_item(control_tree, hf_btle_ll_control_data, tvb, offset + 1, length-1, ENC_NA);
        expert_add_info_format(pinfo, control_item, &ei_unknown_ll_control_code, "Unknown LL Control opcode");
    }
}
...
proto_register_btle(void)
{
    ...
    /* list of fields */
    static const hf_register_info hf[] = {
        { &hf_btle_aa,
            { "Access Address", "nbtle.aa",
            FT_UINT32, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },

        { &hf_btle_pkthdr,
            { "Packet Header", "nbtle.pkthdr",
        ...
        { &hf_btle_adv_addr,
            { "Advertising Address", "nbtle.adv_addr",
            FT_ETHER, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_init_addr,
            { "Init Address", "nbtle.init_addr",
            FT_ETHER, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data,
            { "Advertising Data", "nbtle.adv_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_scan_rsp_data,
            { "Scan Response Data", "nbtle.scan_rsp_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        ...
        { &hf_btle_connect,
            { "Connection Request", "nbtle.connect",
            FT_NONE, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_connect_aa,
            { "Connection Access Address", "nbtle.connect.aa",
            FT_UINT32, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_crc_init,
            { "CRC Init", "nbtle.connect.crc_init",
            FT_UINT24, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_win_size,
            { "Window Size (ms)", "nbtle.connect.win_size",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_win_offset,
            { "Window Offset (ms)", "nbtle.connect.win_offset",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_interval,
            { "Interval (ms)", "nbtle.connect.interval",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_min_interval,
            { "Minimum interval (ms)", "nbtle.connect.min_interval",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_max_interval,
            { "Maximum interval (ms)", "nbtle.connect.max_interval",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_latency,
            { "Latency", "nbtle.connect.latency",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_timeout,
            { "Timeout (ms)", "nbtle.connect.timeout",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_hop_interval,
            { "Hop interval", "nbtle.connect.hop_interval",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_sleep_clock_accuracy,
            { "Sleep Clock Accuracy", "nbtle.connect.sca",
        ...
        // data header
        { &hf_btle_data,
            { "Data PDU Header", "nbtle.data",
            FT_UINT16, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_data_llid,
            { "LLID", "nbtle.data.llid",
            FT_UINT8, BASE_DEC, VALS(llid_codes), 0x3,
            NULL, HFILL }
        },
        { &hf_btle_data_nesn,
            { "NESN", "nbtle.data.nesn",
        ...
        { &hf_btle_instant,
            { "Instant field", "nbtle.instant",
            FT_UINT16, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_channel_map,
            { "Channel map", "nbtle.map",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_error_code,
            { "Error code", "nbtle.error",
            FT_UINT8, BASE_HEX, VALS(error_codes), 0x0,
            NULL, HFILL }
        },
        { &hf_btle_unknown_type,
            { "Unknown type", "nbtle.unknown_type",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_feature_set,
            { "Feature set", "nbtle.feature_set",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_supported_feature,
            { "Supported feature", "nbtle.supported_feature",
            FT_UINT8, BASE_DEC, VALS(le_features), 0x0,
            NULL, HFILL }
        },
        { &hf_btle_unsupported_feature,
            { "Unsupported feature", "nbtle.unsupported_feature",
            FT_UINT8, BASE_DEC, VALS(le_features), 0x0,
            NULL, HFILL }
        },
        { &hf_btle_bt_version,
            { "Bletooth version", "nbtle.bluetooth_version",
            FT_UINT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_company_id,
            { "Company ID", "nbtle.company_id",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_sub_version_num,
            { "Sub-version number", "nbtle.sub_version_num",
            FT_UINT16, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attr_type,
            { "type", "nbtle.adv_data.attr.type",
            FT_UINT8, BASE_HEX, VALS(adv_data_attr_types), 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attr_length,
            { "length", "nbtle.adv_data.attr.length",
            FT_UINT8, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_flags],
            {"flags", "nbtle.adv_data.flags",
            FT_UINT8, BASE_HEX, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_inc_16b_uuids],
            {"16 bit uuids (incomplete)", "nbtle.adv_data.inc_16b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_com_16b_uuids],
            {"16 bit uuids (complete)", "nbtle.adv_data.com_16b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_inc_32b_uuids],
            {"32 bit uuids (incomplete)", "nbtle.adv_data.inc_32b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_com_32b_uuids],
            {"32 bit uuids (complete)", "nbtle.adv_data.com_32b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_inc_128b_uuids],
            {"128 bit uuids (incomplete)", "nbtle.adv_data.inc_128b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_com_128b_uuids],
            {"128 bit uuids (complete)", "nbtle.adv_data.com_128b_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL}
        },
        { &hf_btle_128b_uuid,
            {"128 bit uuid", "nbtle.128b_uuid",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_32b_uuid,
            {"32 bit uuid", "nbtle.32b_uuid",
            FT_UINT32, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_16b_uuid,
            {"16 bit uuid", "nbtle.16b_uuid",
            FT_UINT16, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_short_local_name],
            {"local name (short)", "nbtle.adv_data.short_local_name",
            FT_STRING, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_com_local_name],
            {"local name", "nbtle.adv_data.complete_local_name",
            FT_STRING, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_tx_power],
            {"TX power level", "nbtle.adv_data.tx_power",
            FT_INT8, BASE_DEC, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_dev_class],
            {"device class", "nbtle.adv_data.device_class",
            FT_UINT24, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_pair_hash_c],
            {"simple hash", "nbtle.adv_data.simple_hash",
            FT_UINT16, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_pair_rand_r],
            {"simple randomizer", "nbtle.adv_data.simple_randomizer",
            FT_UINT16, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_dev_id],
            {"TK value", "nbtle.adv_data.tk_value",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_sec_man_oob_flags],
            {"OOB flags", "nbtle.adv_data.oob_flags",
            FT_UINT8, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_conn_int_range],
            {"conn interval range", "nbtle.adv_data.conn_interval_range",
            FT_UINT32, BASE_HEX, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_16b_service_uuids],
            {"16 b service uuids", "nbtle.adv_data.16b_service_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_128b_service_uuids],
            {"128 b service uuids", "nbtle.adv_data.128b_service_uuids",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_service_data],
            {"service data", "nbtle.adv_data.service_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_service_data_32b],
            {"service data for 32 bit UUID.", "nbtle.adv_data.service_data_32b",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_service_data_128b],
            {"service data for 128 bit UUID", "nbtle.adv_data.service_data_128b",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_service_data_value,
            {"service data value", "nbtle.adv_data.service_data.value",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_pub_target_addr],
            {"public target address", "nbtle.adv_data.public_target_address",
            FT_ETHER, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_rand_target_addr],
            {"random target address", "nbtle.adv_data.random_target_address",
            FT_ETHER, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_appearance],
            {"appearance", "nbtle.adv_data.appearance",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attrs[index_hf_btle_adv_data_adv_int],
            {"advertising interval", "nbtle.adv_data.advertising_interval",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_adv_int_ms,
            {"advertising interval (ms)", "nbtle.adv_data.advertising_interval_ms",
            FT_FLOAT, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_manufacturer,
            {"manufacturer specific data", "nbtle.adv_data.manufacturer_data",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_unknown,
            {"Unknown advertising data attribute", "nbtle.adv_data.unknown",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_flag_le_limited_discoverable,
            {"LE limited discoverable", "nbtle.adv_data.flag.le_limited_discoverable",
            FT_BOOLEAN, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_flag_le_general_discoverable,
            {"LE general discoverable", "nbtle.adv_data.flag.le_general_discoverable",
            FT_BOOLEAN, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_flag_br_edr_not_supported,
            {"BR/EDR not supported", "nbtle.adv_data.flag.br_edr_not_supported",
            FT_BOOLEAN, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_flag_simultaneous_le_br_edr_controller,
            {"Simultaneous LE and BR/EDR (Controller)", "nbtle.adv_data.flag.simultaneous_le_br_edr_controller",
            FT_BOOLEAN, BASE_NONE, NULL, 0x0,
            "Simultaneous LE and BR/EDR to Same Device Capable (Controller)", HFILL }
        },
        { &hf_btle_adv_data_flag_simultaneous_le_br_edr_host,
            {"Simultaneous LE and BR/EDR (Host)", "nbtle.adv_data.flag.simultaneous_le_br_edr_host",
            FT_BOOLEAN, BASE_NONE, NULL, 0x0,
            "Simultaneous LE and BR/EDR to Same Device Capable (Host)", HFILL }
        }
#if 0
/* TODO: entries to proto_tree_add_item() */
        { &hf_btle_adv_data_attr_value,
            { "value (hex)", "nbtle.adv_data.attr.value",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        // TODO: proto_tree_add_item()
        { &hf_btle_adv_data_attr,
            { "Attribute", "nbtle.adv_data.attr",
            FT_BYTES, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_adv_data_attr_value_string,
            { "value (ASCII)", "nbtle.adv_data.attr.value_string",
            FT_STRING, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_enabled_channels,
            { "Enabled channels", "nbtle.enabled_channels",
            FT_STRINGZ, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
        { &hf_btle_scan_addr,
            { "Scan Address", "nbtle.scan_addr",
            FT_ETHER, BASE_NONE, NULL, 0x0,
            NULL, HFILL }
        },
#endif
    ...
    // static ei_register_info ei[] = {
        // { &ei_btle_packet_too_short, { "nbtle.length.short", PI_MALFORMED, PI_ERROR, "Packet buffer is too short or reported length is too long.", EXPFILL }},
        // { &ei_btle_packet_too_long, { "nbtle.length.long", PI_MALFORMED, PI_ERROR, "Packet buffer is too long or reported length is too short.", EXPFILL }},
    // };

    //expert_module_t* expert_ip;
    /* register the protocol name and description */
    proto_btle = proto_register_protocol(
        "Bluetooth Low Energy Nordic", /* full name */
        "NBTLE",            /* short name */
        "nbtle"            /* abbreviation (e.g. for filters) */
        );

    register_dissector("nbtle", dissect_btle, proto_btle);
